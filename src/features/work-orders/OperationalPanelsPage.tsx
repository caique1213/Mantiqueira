...
export function OperationalPanelsPage() {
  ...
  const canAccess = auth.hasPermission(config.permission) || auth.hasPermission('work_orders.view.all');
      ...
      {tvMode && (
        <header className={styles.tvHeader}>
          ...
          <div className={styles.tvTools}>
            ...
            <Button size="sm" variant="secondary" leadingIcon={<BellRing />} onClick={silenceForFiveMinutes}>
              Silenciar 5 min
            </Button>
                    ...
                    {columnRows.length ? (
                      columnRows.map((row) => (
                        <WorkOrderPanelCard
                          ...
                          onStart={() => startMutation.mutate(row.work_order_id)}
                        />
                      ))
                    ) : (
    ...
    <div className={styles.osCard} data-priority={row.priority_code}>
      ...
      <div className={styles.cardActions}>
        {row.semantic_state === 'awaiting' && (
          <Button size={tvMode ? 'md' : 'sm'} leadingIcon={<Play />} loading={busy} onClick={onStart}>
            Iniciar
          </Button>
        )}
        <Link to={`/ordens/${row.work_order_id}`}>
